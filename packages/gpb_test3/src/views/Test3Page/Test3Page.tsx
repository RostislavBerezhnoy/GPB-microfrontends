...
import { useModal, Box, WrappedBox, Loader, CalendarCell, errorToastWithButton } from 'gpb_ui'
...
import { dateFormatter, timeFormatter, validateDateBetweenTwoDates } from 'utils/date'
import { MONTH_MODE, YEAR_MODE, DATE_FORMAT } from 'helpers/date'
import './index.css'
...
export const Test3Page = () => {
  ...
  const monthCellRender = (value: Dayjs) => {
    ...
    if (events.length !== 0) return <CalendarCell events={events} />
  }

  const dateCellRender = (value: Dayjs) => {
    const events = getEventsListByDate(value)

    return <CalendarCell events={events} />
  }
